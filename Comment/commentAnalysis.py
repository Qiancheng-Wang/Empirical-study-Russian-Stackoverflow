...
try :
    with connection.cursor() as cursor :
        ...
        while flag1 < len(result1):
            ...
            flag2 = 0
            sql4 = 'select CreationDate from stackcorecomment where userid = %s ;' % thisstackid
            cursor.execute(sql4)
            ...
            sql6 = 'select count(*) from russiancomment where userid = %s ;' % thisrussianid
            cursor.execute(sql6)
            ...
            flag1 += 1
            sql_final = 'insert into commentanalysis values(\'%s\',\'%s\',\'%s\',\'%s\',\'%s\', \'%s\',\'%s\',\'%s\',\'%s\',\'%s\',\'%s\');' % (
