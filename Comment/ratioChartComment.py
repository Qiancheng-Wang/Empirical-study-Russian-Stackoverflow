...
try :
    with connection.cursor() as cursor :
        ...
        sql1 = 'select accountId from commentanalysis ;'
        cursor.execute(sql1)
        ...
        while flag1 < len(result1):
            ...
            sql2 = 'select afterMigrationRussiancomment,beforeMigrationStackcomment,afterMigrationStackcomment,stackcreationdate,russiancreationdate from commentanalysis where accountId = %s;' % thisaccoutid
            cursor.execute(sql2)
            ...
            afterMigrationRussianPost = float(result2['afterMigrationRussiancomment'])
            beforeMigrationStackPost = float(result2['beforeMigrationStackcomment'])
            afterMigrationStackpost = float(result2['afterMigrationStackcomment'])
            ...
            print(proportion)
            flag1 += 1
            sqlfinal = 'insert into russiancommentratio values(\'%s\',\'%s\');' % (thisaccoutid, proportion)
